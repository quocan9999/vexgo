...
- Các quyết định như ghế còn trống, giá vé, khuyến mãi hợp lệ, trạng thái vé, quyền thao tác và phí gửi hàng MUST do backend service quyết định và kiểm tra lại trước khi ghi database; backend MUST xử lý edge case ảnh hưởng đến business rule hoặc tính toàn vẹn dữ liệu.
- Trước khi hoàn tất logic, MUST xem xét edge case phù hợp với feature, không chỉ happy path. Tùy domain, SHOULD cân nhắc input rỗng/null/undefined, boundary value, invalid value, duplicate/unique constraint, dữ liệu không tồn tại, trạng thái không hợp lệ, timezone/date boundary, retry/external failure và concurrency/idempotency khi liên quan.
- MUST NOT thêm xử lý edge case giả định không liên quan chỉ để làm code phức tạp hơn.
...
- Ưu tiên soft-delete hoặc trạng thái phù hợp với domain hiện có thay vì hard-delete dữ liệu nghiệp vụ còn lịch sử hoặc đang được tham chiếu.
...
- Test MUST phát hiện bug/regression và xác minh behavior/contract mong đợi, không chỉ để tăng số lượng test hoặc làm suite pass. Mỗi test SHOULD có khả năng fail với một lỗi thực tế tương ứng; tránh chỉ kiểm tra “không throw”, HTTP 200 hoặc object tồn tại khi behavior quan trọng chưa được xác minh.
- MUST kiểm tra happy path và negative/boundary/edge case quan trọng khi phù hợp. Khi sửa bug, SHOULD thêm regression test tái hiện bug đó.
- MUST NOT nới lỏng assertion chỉ để test pass khi behavior thực tế sai; MUST xác định expected behavior từ business rule/API contract trước khi sửa test.
- Unit tests SHOULD cô lập logic đang kiểm tra và xác minh business behavior, mapping hoặc branch quan trọng. Integration tests SHOULD xác minh sự phối hợp giữa các component thật trong phạm vi test; e2e tests SHOULD kiểm tra flow ở mức HTTP/API gần với client thật. Chỉ mock dependency khi phù hợp với cấp test; MUST NOT mock chính logic/component cần được kiểm tra.
- Số lượng test không phải thước đo chất lượng; MUST NOT tạo test duplicate hoặc trivial chỉ để tăng coverage/test count. MUST NOT đặt business test trong `api-foundation` nếu test thuộc feature riêng.
- Khi review test, SHOULD tự hỏi: “Một bug thực tế nào sẽ làm test này fail?”. Nếu không có câu trả lời rõ ràng, SHOULD xem lại giá trị của test.
