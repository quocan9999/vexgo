...
import { FilterToolbar, SearchInput } from '@/components/data-filters/data-filters';
    ...
    >
      ...
      {detailState.status === 'loading' && (
        <p className="vehicle-types-state" role="status">
          <LoaderCircle aria-hidden="true" className="vehicle-types-spinner" size={17} />
        ...
        <div className="vehicle-type-detail-content">
          <div className="vehicle-type-detail-hero">
            <span className="vehicle-type-detail-mark" aria-hidden="true">
              {detailState.vehicleType.name.slice(0, 1).toLocaleUpperCase('vi')}
            </span>
            <div>
          ...
          <dl className="vehicle-type-detail-fields">
            <div>
              <dt>Mô tả</dt>
              <dd>{detailState.vehicleType.description || 'Chưa có mô tả'}</dd>
            </div>
            <div>
              <dt>Ngày tạo</dt>
...
function sortLabel(
  ...
  if (field !== currentField) return <ArrowUpDown aria-hidden="true" size={14} />;
  ...
  function detailButton(vehicleType: VehicleType) {
    return (
      <Button
        ...
        onClick={() => setSelectedVehicleTypeId(vehicleType.vehicleTypeId)}
        type="button"
        variant="secondary"
      >
          ...
          <div className="panel vehicle-types-panel">
            ...
            {loading && (
              <p className="vehicle-types-loading" role="status">
                <LoaderCircle aria-hidden="true" className="vehicle-types-spinner" size={15} />
                {vehicleTypePage
